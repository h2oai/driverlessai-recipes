...
class MyDateTimeDiffTransformer(CustomTransformer):
    ...
    def transform(self, X: dt.Frame):
        ...
        time1 = pd.to_datetime(pd_t1, format=self.datetime_formats[col1]).fillna(
            pd.Timestamp(year=1970, month=1, day=1)).astype(np.int64)
        time2 = pd.to_datetime(pd_t2, format=self.datetime_formats[col2]).fillna(
            pd.Timestamp(year=1970, month=1, day=1)).astype(np.int64)
        return time1 - time2
