...
class TextBinaryCountTransformer(CustomTransformer):
    ...
    def fit_transform(self, X: dt.Frame, y: np.array = None):
        ...
        self._output_feature_names = ['BinaryCount:' + curr_col + '.' + token.replace(' ', '_') for token in self.count_vec.get_feature_names()]
        self._feature_desc = ["Binary count of '" + token + "' found in " + curr_col for token in self.count_vec.get_feature_names()]
        return X
