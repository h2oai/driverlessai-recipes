...
class DECISION_TREE_PLUS_LINEAR(CustomModel):
    ...
    def _create_tmp_folder(self, logger):
        ...
        tmp_folder = os.path.join(user_dir(), "%s_DTL_model_folder" % uuid.uuid4())
        ...
        for cat in self.leaf_categories:
            if self.is_classifier:
                if (np.mean(y[p == cat]) < 1) and (np.mean(y[p == cat]) > 0):
                    ...
                    model_array[cat] = lm
                    equation_log.append(
                        [[int(round((1 - cat) * sum(p == cat))), int(round(cat * sum(p == cat)))], sum(p == cat),
                         lm.intercept_[0]] + list(lm.coef_[0]))
                else:
