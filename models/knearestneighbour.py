...
class KNearestNeighbourModel(CustomModel):
    ...
    def fit(self, X, y, sample_weight=None, eval_set=None, sample_weight_eval_set=None, **kwargs):
        ...
        if self.num_classes >= 2:
            ...
            model = KNeighborsClassifier(n_neighbors=self.params['n_neighbors'],metric=self.params['metric'],weights=self.params['weights'],n_jobs=self.params['n_jobs'] )            
            lb = LabelEncoder()
            lb.fit(self.labels)
            y = lb.transform(y)
        else:
            model = KNeighborsRegressor(n_neighbors=self.params['n_neighbors'],metric=self.params['metric'],weights=self.params['weights'],n_jobs=self.params['n_jobs'] ) 
        self.means = dict()
