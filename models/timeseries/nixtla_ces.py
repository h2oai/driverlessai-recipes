"""AutoCES for TimeSeries Forecasting 

Uses AutoCES implemented in https://github.com/Nixtla/statsforecast


Current limitations:
(1) No handling for prediction "gap"
(2) Enable user to pass in seasonality length to run AutoCES with seasonality.  NOTE: The seasonality will be infered based on the temporal column.
"""
...
def MyParallelAutoCESTransformer_fit_async(*args, **kwargs):
    return AutoCESParallelModel._fit_async(*args, **kwargs)


def MyParallelAutoCESTransformer_transform_async(*args, **kwargs):
    return AutoCESParallelModel._transform_async(*args, **kwargs)


class AutoCESParallelModel(CustomTimeSeriesModel):
    _regression = True
    _binary = False
    _multiclass = False
    _display_name = "Auto_CES_Parallel"
    _description = "Auto CES TimeSeries forecasting with multi process support"
    _parallel_task = True
    ...
    def set_default_params(
        ...
        #only main parameter to set by user is the seasonality period for CES.
        #if not set defaults to non-seasonal CES
        self.params = dict(
            season_length=kwargs.get("season_length", 1)
        )

    def mutate_params(self, accuracy, time_tolerance, interpretability, **kwargs):
        # No mutation required for any parameters for AutoCES
        pass
        

    @staticmethod
    def _get_n_jobs(logger, **kwargs):
        if "n_jobs_ces" in config.recipe_dict:
            return min(config.recipe_dict["n_jobs_ces"], max_threads())
        try:
            ...
            loggerinfo(logger, "autoces No Max Worker in kwargs. Set n_jobs to 1")
            n_jobs = 1
    ...
    def _clean_tmp_folder(self, logger, tmp_folder):
        try:
            ...
            loggerinfo(logger, "autoces cleaned up temporary file folder.")
        except:
            loggerwarning(
                logger, "autoces could not delete the temporary file folder."
            )
    ...
    def _create_tmp_folder(self, logger):
        ...
        try:
            ...
            loggerwarning(logger, "autoces was denied temp folder creation rights")
            tmp_folder = os.path.join(
                user_dir(), "%s_autoces_model_folder" % uuid.uuid4()
            )
            os.mkdir(tmp_folder)
            ...
            loggerwarning(logger, "autoces temp folder already exists")
            tmp_folder = os.path.join(
                self.context.experiment_tmp_dir,
                "%s_autoces_model_folder" % uuid.uuid4(),
            )
        ...
        loggerinfo(logger, "autoces temp folder {}".format(tmp_folder))
        return tmp_folder

    @staticmethod
    def _fit_async(X_path, grp_hash, tmp_folder, exogenous_vars, params): #, cap):
        """
        Fits a autoces model for a particular time group
        :param X_path: Path to the data used to fit the autoces model
        :param grp_hash: Time group identifier
        :exogenous_vars: column names of additional predictors, AutoCES does not use them
        ...
        # print("autoces - fitting on data of shape: %s for group: %s" % (str(X.shape), grp_hash))
        #if X.shape[0] < 20:
        #    return grp_hash, None

        # Import autoces model
        mod = importlib.import_module("statsforecast.models")        
        model = mod.AutoCES(season_length = params["season_length"])  
        
        with suppress_stdout_stderr():
            try:
                model.fit(X['y'].values)
            except:
                model = None # model fit failed for AutoCES
                
        model_path = os.path.join(tmp_folder, "autoces_model" + str(uuid.uuid4()))
    ...
    def fit(
        ...
        loggerinfo(
            logger, "Start Fitting autoces Model with params : {}".format(self.params)
        )

        try:
            # Add value of autoces_top_n in recipe_dict variable inside of config.toml file
            # eg1: recipe_dict="{'autoces_top_n': 200}"
            # eg2: recipe_dict="{'autoces_top_n':10}"
            self.top_n = config.recipe_dict["autoces_top_n"]
        except KeyError:
            self.top_n = 50

        loggerinfo(
            logger,
            f"autoces will use {self.top_n} groups as well as average target data.",
        )
        ...
        # Change date feature name to match autoces requirements
        ...
        # Send that to autoces
        mod = importlib.import_module("statsforecast.models")
        model = mod.AutoCES(season_length=self.params["season_length"])  

        with suppress_stdout_stderr():
            try:
                model.fit(X_avg['y'].values) #AutoCES accepts only numpy arrays
            except:
        ...
        if top_groups:
            ...
            loggerinfo(logger, f"autoces will use {n_jobs} workers for fitting.")
            ...
            # Fit 1 autoces model per time group column
            ...
            for _i_g, (key, X) in enumerate(X_groups):
                ...
                X_path = os.path.join(tmp_folder, "autoces" + str(uuid.uuid4()))
                ...
                pool.submit_tryget(
                    None,
                    MyParallelAutoCESTransformer_fit_async,
                    args=args,
                    kwargs=kwargs,
                    out=grp_models,
                )
    ...
    def _transform_async(model_path, X_path, nan_value, exogenous_vars, tmp_folder):
        ...
        XX_path = os.path.join(tmp_folder, "autoces_XXt" + str(uuid.uuid4()))
        X = load_obj(X_path)
        # autoces returns the predictions ordered by time
        ...
        if model is not None:
            try:
                ...
                ) # AutoCES generated error while predicting
    ...
    def predict(self, X: dt.Frame, **kwargs):
        ...
        ### AutoCES returns the results in sorted time order so allow for that
        ...
        if top_groups:
            ...
            for _i_g, (key, X_grp) in enumerate(X_groups):
                ...
                if (_i_g + 1) % max(1, nb_groups // 20) == 0:
                    loggerinfo(
                        logger,
                        "autoces : %d%% of groups predicted"
                        % (100 * (_i_g + 1) // nb_groups),
                    )

                # Create dict key to store the min max scaler
                grp_hash = self.get_hash(key)
                X_path = os.path.join(tmp_folder, "autoces_Xt" + str(uuid.uuid4()))
                ...
                model_path = os.path.join(
                    tmp_folder, "autoces_modelt" + str(uuid.uuid4())
                )
                save_obj(model, model_path)
                ...
                pool.submit_tryget(
                    None,
                    MyParallelAutoCESTransformer_transform_async,
                    args=args,
                    kwargs=kwargs,
                    out=XX_paths,
                )
