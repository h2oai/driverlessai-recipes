"""autoarima for TimeSeries with an example of parameter mutation."""
import importlib
...
class AutoARIMAParallelModel(CustomTimeSeriesModel):
    ...
    _binary = False
    _multiclass = False
    _display_name = "AutoARIMA_Parallel"
    _description = "AutoARIMA TimeSeries forecasting with multi process support"
    _parallel_task = True
    ...
    def _fit_async(X_path, grp_hash, tmp_folder):
        ...
        :param X_path: Path to the data used to fit the Prophet model
        ...
        # Import AutoARIMA package
        mod = importlib.import_module("statsforecast.adapters.prophet")
        Prophet = getattr(mod, "AutoARIMAProphet")
    ...
    def _get_n_jobs(logger, **kwargs):
        if "n_jobs_autoarima" in config.recipe_dict:
            return min(config.recipe_dict["n_jobs_autoarima"], max_threads())
        try:
    ...
    def _fit_async(X_path, grp_hash, tmp_folder, params, cap):
        ...
        if params["monthly_seasonality"]:
            model.add_seasonality(
                name="monthly", period=30.5, fourier_order=params["monthly_seasonality"]
            )
    ...
    def fit(
        ...
        **kwargs,
        ...
        if top_groups:
            ...
            #
            # Fit 1 autoarima model per time group columns
        ...
        self.set_model_properties(
            ...
            features=self.tgc,  # autoarima uses time and timegroups
